...
class ByteBuffer
{
 ...
 public:
  ...
  /// buffer中的内容为offset处开始的size大小的缓冲
  ByteBuffer &assign(const char *data, uint32_t offset, uint32_t size);
  /// buffer中的内容为offset处开始的size大小的缓冲，并且buffer并不负责内存的释放
 ...
 public:
  // generic put & get
  template<typename T>
  ByteBuffer &put(const T &e) throw(out_of_range);
  template<typename T>
  ByteBuffer &get(T &e) throw(out_of_range);
  template<typename T>
  ByteBuffer &put(const std::vector<T> &v) throw(out_of_range);
  template<typename T>
  ByteBuffer &get(std::vector<T> &v) throw(out_of_range);
  template<typename T>
  /// 实际上调用的是put函数
  ByteBuffer &operator<<(const T &e) throw(out_of_range) { return put(e); }
  template<typename T>
  /// 实际上调用的是get函数
  ByteBuffer &operator>>(T &e) throw(out_of_range) { return get(e); }

  template<typename T>
  const ByteBuffer &peek(T &e) const throw(out_of_range);

  template<typename T>
  T get() throw(out_of_range);
  ...
  template<typename T>
  ByteBuffer &getRef(int index, T *&dst) throw(out_of_range);
  template<typename T>
  ByteBuffer &getRef(int index, const T *&dst) throw(out_of_range);
...
/// 首先存放vector的大小，再依次存放vector中的每个元素
template<typename T>
